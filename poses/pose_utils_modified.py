...
def load_colmap_data(realdir):
    ...
    camerasfile = r"D:\IDP 4th year\colmap\workspace\sparse\0\cameras.bin"
    print(camerasfile)
    ...
    imagesfile = r"D:\IDP 4th year\colmap\workspace\sparse\0\images.bin"
    ...
    points3dfile = r"D:\IDP 4th year\colmap\workspace\sparse\0\points3D.bin"
    pts3d = read_model.read_points3d_binary(points3dfile)
    
    # must switch to [-u, r, -t] from [r, -u, t], NOT [r, u, -t]
    poses = np.concatenate([poses[:, 1:2, :], poses[:, 0:1, :], -poses[:, 2:3, :], poses[:, 3:4, :], poses[:, 4:5, :]], 1)
    
    return poses, pts3d, perm,c2w_mats
...
def gen_poses(basedir , factors=None):
    ...
    poses, pts3d, perm,c2w_mats = load_colmap_data(basedir)
    ...
    return c2w_mats, perm,poses
    
if __name__=="__main__":
    c2w_mats,perm,poses = gen_poses(r"D:\IDP 4th year\colmap\workspace")
    # c2w_mats = c2w_mats[perm]
    # # poses = poses[perm]
    # print(poses.shape)
    # print(poses)

    # np.save(r"D:\IDP 4th year\colmap\workspace\train_poses.npy", c2w_mats)

    # print(poses[:,:,0])
    # poses = np.reshape(poses, (-1, 100)).T
    # print(perm)
    # print(poses.shape)
    # c2w_matrices = [np.vstack([np.reshape(x, (3, 4)), np.array([0, 0, 0, 1])]) for x in poses[:, :12]]
    # for i in range(100) :
    #     print(c2w_matrices[i])
    #     print("\n")
